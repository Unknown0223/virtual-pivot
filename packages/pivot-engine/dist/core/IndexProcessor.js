...
import { aggregationForColumn, columnTotals, measureGrandTotals, rowTotals, valueFieldIdFromColumnKey } from "./aggregationColumnUtils.js";
function toIndexCell(cell, numerator, rowTotal, colTotal, measureTotal, valueDef) {
    if (numerator == null ||
        rowTotal == null ||
        colTotal == null ||
        measureTotal == null ||
...
function processRowCells(row, config, colTotals, measureTotals, valueDefMap) {
    row.cells = row.cells.map((cell) => {
        if (cell.columnKey === "__row_label__")
            return cell;
        if (aggregationForColumn(cell.columnKey, config) !== "INDEX")
            return cell;
        const fieldId = valueFieldIdFromColumnKey(cell.columnKey, config);
        const valueDef = fieldId ? valueDefMap.get(fieldId) : undefined;
        if (!valueDef || !fieldId)
            return cell;
        return toIndexCell(cell, cell.rawValue, rowTotals(row, fieldId, config), colTotals.get(cell.columnKey) ?? null, measureTotals.get(fieldId) ?? null, valueDef);
    });
...
function processTotalRow(total, config, colTotals, measureTotals, valueDefMap, parentRow) {
    return {
        ...
        cells: total.cells.map((cell) => {
            ...
            return toIndexCell(cell, cell.rawValue, rowTotal, colTotals.get(cell.columnKey) ?? null, measureTotals.get(fieldId) ?? null, valueDef);
        })
    };
}
/** INDEX — (value × grand total) / (row total × column total). SUM asosida. */
export function applyIndexAggregations(data, config) {
