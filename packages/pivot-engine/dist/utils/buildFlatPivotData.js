 ...
 */
export function getFlatColumnFieldIds(config) {
    const ordered = [
        ...config.rows,
        ...config.columns,
...
function formatCellValue(raw, field, showCurrency) {
    if (raw == null || raw === "") {
 ...
 */
export function buildFlatPivotData(rawData, fields, config, startTime = performance.now()) {
    const warnings = [];
    ...
    if (config.options.maxRows && workingData.length > config.options.maxRows) {
        warnings.push(getPivotStrings().reportBuilder.pivotRowsTruncated(String(config.options.maxRows), String(workingData.length)));
    ...
    const rows = workingData.map((row, index) => {
    ...
    if (config.options.showGrandTotal && workingData.length > 0) {
        const cells = columnIds.map((id, colIdx) => {
            const field = fieldMap.get(id);
