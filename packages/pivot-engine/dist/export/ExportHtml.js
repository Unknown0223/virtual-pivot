...
export function pivotDataToHtml(data, options = {}) {
    ...
    const thead = headRows
        .map((row) => `<tr>${row.map((cell) => `<th>${escapeHtml(String(cell ?? ""))}</th>`).join("")}</tr>`)
        .join("");
    const tbody = bodyRows
        .map((row, idx) => `<tr class="${idx === bodyRows.length - 1 && data.grandTotal ? "grand-total" : ""}">${row
        .map((cell, ci) => {
...
export async function exportPivotToHtml(data, options = {}) {
