...
function formatExportCell(value: unknown): string | number {
...
export function exportRawRecordsToExcel(
  records: Record<string, unknown>[],
  columns: RawRecordColumn[],
  options: ExportRawRecordsOptions = {}
): void {
  ...
  const rows = records.map((record) =>
    columns.map((col) => formatExportCell(record[col.id]))
  );
...
export function exportRawRecordsToCsv(
  records: Record<string, unknown>[],
  columns: RawRecordColumn[],
  ...
  const lines = [
    ...
    ...records.map((record) =>
      columns.map((col) => escape(String(formatExportCell(record[col.id])))).join(",")
    )
  ];
