...
export function pivotDataToPdfTable(
  data: PivotData,
  ...
  if (aoa.length === 0) return { head: [], body: [] };
...
export function countPdfExportRows(data: PivotData, expandedRows?: Set<string>): number {
 ...
 */
export async function exportPivotToPdf(
  data: PivotData,
  options: ExportPdfOptions = {}
): Promise<void> {
