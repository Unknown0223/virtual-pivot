...
export function isWdrMeasuresFieldId(fieldId: string): boolean {
  const key = fieldId.trim().toLowerCase();
  return WDR_MEASURES_NAMES.has(key);
}

/** Default: columns (classic pivot — measures side by side). */
export function resolveValuesPosition(options?: PivotOptions): PivotValuesPosition {
  return options?.valuesPosition === "rows" ? "rows" : "columns";
}

export function valuesOnRows(options?: PivotOptions): boolean {
