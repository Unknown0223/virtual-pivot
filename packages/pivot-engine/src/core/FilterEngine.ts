...
export class FilterEngine {
  apply(
    ...
    const rankFilters = filters.filter(
      (filter) => filter.type === "top_n" || filter.type === "bottom_n"
    );
    ...
    let result = data.filter((row) =>
      rowFilters.every((filter) => this.matchesFilter(row, filter, fields))
    );
  ...
  private applyRankFilter(
    ...
    if (!limit || limit <= 0 || !data.length) return data;

    const scores = new Map<string | number, number>();

    for (const row of data) {
      const key = row[filter.fieldId] as string | number;
    ...
    const ranked = [...scores.entries()].sort((a, b) =>
      filter.type === "bottom_n" ? a[1] - b[1] : b[1] - a[1]
    );
  ...
  private rankScore(row: Record<string, unknown>, filter: PivotFilter): number {
  ...
  private matchesFilter(
    ...
    switch (filter.type) {
      case "include":
        return filter.values?.includes(value as string | number) ?? true;

      case "exclude":
        return !(filter.values?.includes(value as string | number) ?? false);
  ...
  private toDate(value: unknown, field?: PivotField): Date | null {
